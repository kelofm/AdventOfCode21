...
fn main()
{
    ...
    if let Ok(file) = std::fs::File::open("input") {
        for line in io::BufReader::new(&file).lines().flatten().filter(|l| l != "") {
            let item = parseList(&line, 1);
            match items.binary_search(&item.0) {
                Ok(position) => {
                    items.insert(position, item.0.clone());
                },
                Err(position) => {
                    items.insert(position, item.0.clone());
                }
    ...
    let begin = Item::List(Vec::from([
        Item::List(Vec::from([
            Item::Int(2)
        ]))
    ]));

    let end = Item::List(Vec::from([
        Item::List(Vec::from([
            Item::Int(6)
        ]))
    ]));
    ...
    for (i, item) in [begin, end].iter().enumerate() {
        match items.binary_search(&item) {
            Ok(position) => {output *= position + 1 + i;}
            Err(position) => {output *= position + 1 + i;}
        }
    }
