...
impl Monkey
{
    ...
    {
        self.items.push(item % self.modulo);
    }
    ...
    fn monkeyBusiness(&mut self) -> Vec<(usize,i64)>
    {
        ...
        for item in self.items.iter().copied() {
                //print!("inspect {}", item);
                let value = self.operation.as_ref()(item);
                //print!(" => {}", value);
...
fn main()
{
    ...
    let modulo = monkeys.iter().map(|m| m.getTest()).product::<i64>();
    for i_monkey in 0..monkeys.len() {
        monkeys[i_monkey].setMod(modulo);
