...
fn drop(occupied: &HashSet<(i64,i64)>, boundaries: &HashSet<(i64,i64)>, floor: i64) -> (i64, i64)
{
    ...
    'outer: loop {
        for newPosition in [(position.0, position.1 + 1), (position.0 - 1, position.1 + 1), (position.0 + 1, position.1 + 1)].iter() {
            if !occupied.contains(newPosition) && newPosition.1 < floor {
...
fn main()
{
    ...
    if let Some(max) = boundaries.iter().map(|p| p.1).max() {
        let floor = max + 2;
        for i_drop in 0.. {
            match drop(&occupied, &boundaries, floor) {
                (500, 0) => {
                    println!("{}", i_drop + 1);
                    break;
                },
